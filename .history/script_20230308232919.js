...
btns.forEach(btn => {
    btn.addEventListener('click', e => {
        ...
        fetch(`http://backend4theforkofit.herokuapp.com/api/categories/name/${btnName}`)
        .then(function(res){
            return res.json()
        })
        .then(function (data) { 
            ...
            if (category.hasSubSection === true) {
                fetch(`http://backend4theforkofit.herokuapp.com/api/subCategories/category/${catID}`)
                .then( res => res.json())
                .then( data => {
                    subCategories = data
                    ...
                    subCategories.forEach( subCat => {
                        ...
                        fetch(`http://backend4theforkofit.herokuapp.com/api/products/subCategory/${subCatID}`)
                        .then( res => res.json() )
                        .then( data => {
                            products = data
                ...
                fetch(`http://backend4theforkofit.herokuapp.com/api/products/category/${catID}`)
                .then( res => res.json() )
                .then( data => {
                    products = data
                    // console.log(products);
