...
btns.forEach(btn => {
    btn.addEventListener('click', e => {
        ...
        fetch(`http://localhost:5000/api/categories/name/${btnName}`)
        .then(function(res){
            return res.json()
        })
        .then(function (data) { 
            ...
            newSectTitle.innerHTML = btn.firstElementChild.innerText;
            // Section Description
            ...
            if (category.hasSubSection === true && category.isTray === true) {
                fetch(`http://localhost:5000/api/subCategories/category/${catID}`)
                .then( res => res.json())
                .then( data => {
                    subCategories = data
                    ...
                    subCategories.forEach( subCat => {
                        ...
                        fetch(`http://localhost:5000/api/products/subCategory/${subCatID}`)
                        .then( res => res.json() )
                        .then( data => {
                            products = data
